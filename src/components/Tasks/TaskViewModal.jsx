    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-2 sm:p-4">
      <div className="bg-white rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto shadow-xl">
        ...
        <div className="flex justify-between items-center p-4 sm:p-6 border-b border-gray-200">
          <h2 className="text-xl sm:text-2xl font-bold text-[#E4007C]">📋 Task Overview</h2>
          <button onClick={onClose} className="hover:bg-gray-100 rounded-full p-2 transition">
        ...
        <div className="p-4 sm:p-6 space-y-6">
          ...
          <div className="space-y-1 sm:space-y-2">
            <h3 className="text-lg sm:text-xl font-semibold text-gray-900">{task.title}</h3>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-pink-50 p-4 rounded-xl text-sm">
            <div>
              <label className="block text-gray-700 font-medium mb-1">Total Videos Required</label>
            ...
            <div>
              <label className="block text-gray-700 font-medium mb-1">Uploaded Videos</label>
              <p className="font-semibold text-gray-900">{task.uploadedVideos}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="text-gray-800">{new Date(task.endDate).toLocaleString()}</p>
            </div>
          </div>

          {/* Videos */}
          <div>
            <h4 className="text-lg font-semibold text-gray-900 mb-3">🎞️ Submitted Videos</h4>
                ...
                {currentVideos.map((video) => (
                  ...
                  >
                    <div className="space-y-1">
                      <p className="text-sm sm:text-base font-medium text-gray-900">Video ID: {video.Video_ID}</p>
                      <p className="text-sm text-gray-600">Score: {video.Score ?? "N/A"}</p>
                      <p className="text-sm text-gray-500">Updated At: {video.Update_AT}</p>
                    </div>
